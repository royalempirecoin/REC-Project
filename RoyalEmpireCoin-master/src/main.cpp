...
#if defined(NDEBUG)
# error "RoyalEmpireCoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "RoyalEmpireCoin Signed Message:\n";
...
int CMerkleTx::GetBlocksToMaturity() const
{
    if (!(IsCoinBase() || IsCoinStake()))
        return 0;
    return max(0, (nCoinbaseMaturity+45) - GetDepthInMainChain());
}
...
{
    int64_t nSubsidy = 50 * COIN; // RomanEmpireCoin base reward

    // Premine
    if(nHeight == 1) {
        nSubsidy = 4333320 * COIN;
    }
...
unsigned int DarkGravityWave(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Modified for RoyalEmpireCoin: softfork block set to min diff to re-engage chain movement
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    //LogPrintf("RoyalEmpireCoin retargetted using: DGW-v3 difficulty algo \n");
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal royalempirecoin miner
...
#ifdef ENABLE_WALLET
// royalempirecoin: attempt to generate suitable proof-of-stake
...
{
    RenameThread("royalempirecoin-loadblk");
