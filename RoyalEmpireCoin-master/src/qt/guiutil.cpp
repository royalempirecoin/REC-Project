...
#include "royalempirecoinaddressvalidator.h"
...
QFont royalempirecoinAddressFont()
{
...
{
    widget->setMaxLength(RoyalEmpireCoinAddressValidator::MaxAddressLength);
    widget->setValidator(new RoyalEmpireCoinAddressValidator(parent));
    widget->setFont(royalempirecoinAddressFont());
}
...
bool parseRoyalEmpireCoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // NovaCoin: check prefix
    if(uri.scheme() != QString("royalempirecoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!RoyalEmpireCoinUnits::parse(RoyalEmpireCoinUnits::REC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseRoyalEmpireCoinURI(QString uri, SendCoinsRecipient *out)
{
    // Convert royalempirecoin:// to royalempirecoin:
    //
    //    Cannot handle this later, because royalempirecoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("royalempirecoin://"))
    {
        uri.replace(0, 12, "royalempirecoin:");
    }
    QUrl uriInstance(uri);
    return parseRoyalEmpireCoinURI(uriInstance, out);
}
...
void openConfigfile()
{
    ...
    /* Open royalempirecoin.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "RoyalEmpireCoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for RoyalEmpireCoin.lnk
...
{
    return GetAutostartDir() / "royalempirecoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a royalempirecoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=RoyalEmpireCoin\n";
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("RoyalEmpireCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  royalempirecoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("RoyalEmpireCoin-Qt"));
...
void SetBlackThemeQSS(QApplication& app)
{
    app.setStyleSheet("QWidget        { background: rgb(41,44,48); }"
                      ...
                      "QPushButton    { background: rgb(0,189,21); color: rgb(21,21,21); }"
                      ...
                      "QToolButton:checked { color: rgb(255,255,255); border: none; border-left-color: rgb(0,127,14); border-left-style: solid; border-left-width: 6px; }"
