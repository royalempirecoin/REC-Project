...
{
    QString ret = QString("royalempirecoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non REC unit input in lnReqAmount, we generate the URI with REC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(RoyalEmpireCoinUnits::format(RoyalEmpireCoinUnits::REC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
