...
void ThreadStakeMiner(CWallet *pwallet)
{
    ...
    RenameThread("royalempirecoin-miner");
...
void static RoyalEmpireCoinMiner(CWallet *pwallet)
{
    LogPrintf("RoyalEmpireCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("royalempirecoin-miner");
	...
	CBlock *pblock = pblocktemplate.get();
        ...
        LogPrintf("Running RoyalEmpireCoinMiner with %llu transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("RoyalEmpireCoinMiner terminated\n");
        throw;
    }
}

void GenerateRoyalEmpireCoins(bool fGenerate, CWallet* pwallet, int nThreads)
{
	...
	minerThreads->create_thread(boost::bind(&RoyalEmpireCoinMiner, pwallet));
}
#endif
